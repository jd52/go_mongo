...
func ListDevice(qy *Device, andOr *string, w http.ResponseWriter, r *http.Request) []Device {
	...
	if *andOr == "and" {
		...
		err = deviceCollect.Find(bson.M{"$or": []bson.M{bson.M{"hostname": bson.RegEx{Pattern: "$regex/" + qy.Hostname + "/", Options: ""}}, bson.M{"ipaddress": qy.IPAddress}, bson.M{"devicetype": qy.DeviceType}}}).All(&result)
		fmt.Println(qy)
	}
	if err != nil {
		log.Fatal(err)
	...
	fmt.Println(bson.M{"hostname": bson.RegEx{Pattern: "$regex:/" + qy.Hostname + "/", Options: ""}})
	return result
}
